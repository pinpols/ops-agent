...
| **最小权限** | 非 root(uid 10001)、`readOnlyRootFilesystem`、`drop ALL` caps;只写 emptyDir(`/var/run/ops-agent`、`/app/.ops-agent`)。 |
...
- **NetworkPolicy**:限制 ingress/worker 只能访问 Redis、LLM、目标系统;worker egress allowlist。
