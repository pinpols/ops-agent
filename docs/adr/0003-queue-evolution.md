...
**热路径原子性**(审计后补强):Redis 的 `submit`/`dlq_requeue`/`fail_or_retry`/`complete` 全用 WATCH/MULTI + 存在/终态守卫 —— 否则 `hincrby` 在 hash TTL 过期后会重建无 question 的僵尸 job、已 SUCCEEDED 的 job 会被迟到 retry 覆写回 QUEUED。
...
- ⚠️ at-least-once → 任务可能重复处理;靠终态守卫 + 幂等副作用消化。
